...
* Per-provider HTTP/TLS options for the `openai` LangChain provider via `[models.providers.openai.http]` in `~/.deepagents/config.toml` (`verify_ssl`, `ca_bundle`, `timeout`, `trust_env`), wiring custom `httpx` clients into `init_chat_model` when configured. See `libs/cli/docs/provider-http-tls.md`.
